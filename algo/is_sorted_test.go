...
func TestIsSortedUntil(t *testing.T) {
	t.Run("Normal", func(t *testing.T) {
		...
		it := algo.IsSortedUntilOrdered[int](s.Begin(), s.End())
...
func TestIsSorted(t *testing.T) {
	s := slice.Of[int]()
	assert.True(t, algo.IsSortedOrdered[int](s.Begin(), s.End()))
	s = slice.Of(1)
	assert.True(t, algo.IsSortedOrdered[int](s.Begin(), s.End()))
	s = slice.Of(1, 2)
	assert.True(t, algo.IsSortedOrdered[int](s.Begin(), s.End()))
	...
	assert.False(t, algo.IsSortedOrdered[int](s.Begin(), s.End()))
	s = slice.Of(2, 2, 2, 1)
	assert.False(t, algo.IsSortedOrdered[int](s.Begin(), s.End()))
}
